...
/**
 * VertexList UDF intakes property graph elements and spits out vertex list
 * elements which are tuples of (vertex id, vertex label). For example,
 * (Employee001, OWL.People) If the constructor parameter is set to "TRUE",
 * "true" or "1", the vertex list element also contains edge properties returned
 * as key value pair, e.g. (Employee002, OWL.People, name:Alice, age:30)
 * 
 */
...
public class VertexList extends EvalFunc<String> {
     ...
     *            Print vertex properties if set to "1", "TRUE" or "true", does
     *            not print vertex properties if set to "0", "FALSE", "false"
     */
    ...
    public String exec(Tuple input) throws IOException {
        ...
        if (graphElement.isVertex()) {
            ...
            builder.append(vertex.getId().getName().toString());
            if (vertex.getLabel() != null) {
                builder.append('\t');
                builder.append(vertex.getLabel().toString());
            }
            if (this.printProperties) {
                builder.append('\t');
                builder.append(vertex.getProperties().toString("=", "\t"));
            }
