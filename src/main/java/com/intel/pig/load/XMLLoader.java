...
/**
 * Extend ByteArrayOutputStream to have access to its underlying buffer and
 * instead of toByteArray() calls use getByteArray() to get rid of extra memory
 * allocations
 */
 ...
 * <code>BufferedPositionedInputStream<code>
 ...
 * methods and fields. It also provides additional method
 * <code>collectTag<collect> which will give the byte 
 * array between the tag which is a xml record. i.e <tag> .*</tag> will be returned
 * 
 * @note we can't use the standard SAX or STAX parser as for a big xml the
 *       intermittent hadoop block may not be the valid xml and hence those
 *       parser may create pb.
 * 
 * @since pig 2.0
 */

class XMLLoaderBufferedPositionedInputStream extends
	...
	/**
	 * The field set the maximum bytes that is readable by this instance of
	 * stream.
	 */
	...
	/**
	 * Creates a <code>XMLLoaderBufferedPositionedInputStream</code> by
	 * assigning the argument <code>in</code> to the field
	 * <code>this.wrapperIn</code> so as to remember it for later use.
	 * 
	 * @param in
	 ...
	 *            the underlying input stream
	 * @param start
	 *            start location of the split
	 * @param end
	 *            end location of the split
	 */
	public XMLLoaderBufferedPositionedInputStream(InputStream in, long start,
			long end) {
		this(in);
		this.end = end;
	...
	/**
	 * Set the stream readable or non readable. This is needed to control the
	 * xml parsing.
	 * 
	 * @param flag
	...
	/**
	 * See if the stream readable or non readable. This is needed to control the
	 * xml parsing.
	 * 
	...
	/**
	 * org.apache.pig.impl.io.BufferedPositionedInputStream.read It is just the
	 * wrapper for now. Reads the next byte of data from this input stream. The
	...
	/**
	 * This is collect the bytes from current position to the ending tag. This
	 * scans for the tags and do the pattern match byte by byte this must be
	 * used along with XMLLoaderBufferedPositionedInputStream#skipToTag
	 * 
	 * @param tagName
	 *            the end tag to search for
	 * 
	 * @param limit
	 *            the end pointer for the block for this mapper
	 * 
	...
	private byte[] collectUntilEndTag(String tagName, long limit) {
		...
		// @todo use the charset and get the charset encoding from the xml
		// encoding.
		byte[] tmp = tagName.getBytes();
		// ByteArrayOutputStream collectBuf = new ByteArrayOutputStream(1024);
		_ByteArrayOutputStream collectBuf = new _ByteArrayOutputStream(1024);
		...
		// the matched element might contain nested element, we keep track of
		// the
		...
		return collectBuf.getByteArray();
	}

	/**
	 * This is collect the from the matching tag. This scans for the tags and do
	 * the pattern match byte by byte This returns a part doc. it must be used
	 * along with XMLLoaderBufferedPositionedInputStream#collectUntilEndTag
	 * 
	 * @param tagName
	...
	private byte[] skipToTag(String tagName, long limit) throws IOException {
		...
		_ByteArrayOutputStream matchBuf = new _ByteArrayOutputStream(512);
		int idxTagChar = 0;
		int state = S_START;

		/*
		 * Read till the tag is found in this block. If a partial tag block is
		 * found then continue on to the next block.matchBuf contains the data
		 * that is currently matched. If the read has reached the end of split
		 * and there are matched data then continue on to the next block.
		 */
		while (splitBoundaryCriteria(wrapperIn) || (matchBuf.size() > 0)) {
			...
			try {
				...
				case S_START:
					...
					break;
				case S_MATCH_PREFIX:
					// tag match iff next character is whitespaces or close tag
					// mark
				...
				if (state == S_MATCH_TAG
						&& (b == '>' || Character.isWhitespace(b))) {
					break;
				}
		...
		return matchBuf.getByteArray();
	}

	/**
	 * Returns whether the split boundary condition has reached or not. For
	 * normal files ; the condition is to read till the split end reaches. Gz
	 * files will have maxBytesReadable set to near Long.MAXVALUE, hence this
	 * will cause the entire file to be read. For bz2 and bz files, the
	 ...
	 * @throws IOException
	 */
	private boolean splitBoundaryCriteria(InputStream wrapperIn2)
			throws IOException {
	...
	/**
	 * This is collect bytes from start and end tag both inclusive This scans
	 * for the tags and do the pattern match byte by byte
	 * 
	 * @param tagName
	 *            the start tag to search for
	 * 
 ...
 * functions to cast raw byte data into various datatypes. other sections of the
 * code can call back to the loader to do the cast. This takes a xmlTag as the
 * arg which it will use to split the inputdataset into multiple records. <code>
 *    
...
public class XMLLoader extends LoadFunc {
	...
	/**
	 * Constructs a Pig loader that uses specified string as the record
	 * seperater for example if the recordIdentifier is document. It will
	 * consider the record as <document> .* </document>
	 * 
	 * @param recordIdentifier
	 *            the xml tag which is used to pull records
	 * 
	 */
	...
	public InputFormat getInputFormat() throws IOException {
		XMLFileInputFormat inputFormat = new XMLFileInputFormat(
				recordIdentifier);
	...
	public static class XMLFileInputFormat extends FileInputFormat {
		...
		/**
		 * Boolean flag used to identify whether splittable property is
		 * explicitly set.
		 */
	...
	public static class XMLFileRecordReader extends RecordReader {
		...
		public void initialize(InputSplit genericSplit,
			...
			if (file.toString().endsWith(".bz2")
					|| file.toString().endsWith(".bz")) {
				// For bzip2 files use CBZip2InputStream to read and supply the
				// upper input stream.
				CBZip2InputStream in = new CBZip2InputStream(fileIn, 9, end);
				this.xmlLoaderBPIS = new XMLLoaderBufferedPositionedInputStream(
						in, start, end);
				...
				if (codec != null) {
					...
					CompressionInputStream stream = codec
							.createInputStream(fileIn);
					this.xmlLoaderBPIS = new XMLLoaderBufferedPositionedInputStream(
							stream, start, end);
				}
			}
