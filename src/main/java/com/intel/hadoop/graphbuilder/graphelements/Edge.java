...
/**
 * Represents an Edge object with a source, destination, label, and a (possibly
 * empty) property map.
 * <p>
...
public class Edge<VidType extends WritableComparable<VidType>> extends GraphElement implements Writable {
    ...
    /**
     * Creates an edge with given source name, source label, destination name,
     * destination label, and edge label..
     * 
     * @param srcName
    ...
    /**
     * Creates an edge with given source name, source label, destination name,
     * destination label, edge label, and property map
     * 
     * @param srcName
     *            the name of the source vertex.
     * @param srcLabel
    ...
    /**
     * Checks to see if this edge is null. If any of the values are nulls the
     * whole thing is null. If we try to write an edge with any null values
     * Graph Builder will throw an exception.
     * 
     * @return True or false, based upon the null status of the src, dst, and
     *         label.
     */
    @Override
     ...
     * 
     * @param {@code key} The lookup key for the value in the edge's property
     *        map.
     */
     ...
     * @param {@code key} The lookup key for the value in the edge's property
     *        map.
     * @param {@code val} The value to add to the property map (if the key is
     *        not there already) and that we overwrite the existing value (if
     *        the key is already in the property map).
     */
    ...
    /**
     * Determines if the edge is a loop - that is, if its source and destination
     * are the same vertex.
     * 
    ...
    /**
     * Sets the entire property map for the edge. Overwrites the existing
     * property map.
     * 
    ...
    /**
     * Checks if the input edge is equal to the current object. This is a deep
     * check which means source, destination vertex ID's and all properties are
     * checked to decide equality
     * 
     * @param object
     *            an object being tested for equality with the edge
     */
    @Override
    public boolean equals(Object object) {
        if (object instanceof Edge) {
            ...
            return (this.src.equals(edge.getSrc()) && this.dst.equals(edge.getDst()) && this.label.equals(edge.getLabel()) && this.properties
                    .equals(edge.getProperties()));
     ...
     * 
     * @return The triple of the edge's source vertex ID, destination vertex ID,
     *         and its label.
     */
    public EdgeID getId() {
    ...
    /**
     * Converts an edge into a string for printing. The properties are tab
     * separated.
     * 
    ...
    public final String toString() {
        return src.toString() + "-> " + dst.toString() + "(Label: " + label.toString() + " - Properties: ["
                + properties.toString() + "])";
    }
