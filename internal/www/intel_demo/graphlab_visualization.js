...
var domain_str = "http://10.23.61.9:8090";
...
//    console.log(data);    
    ...
    if (data.phase_name == "Preprocessing XML") {
		$("#graph_builder_phase_name").css("background", "#126AEE");
		// $("#graph_builder_progress").progressbar("option", "disabled", false);
	} else if (data.phase_name == "Partitioning graphs") {
		$("#graph_builder_phase_name").css("background", "#3F15C4");
		// $("#graph_builder_progress").progressbar("option", "disabled", false);
	} else if (data.phase_name == "Finalizing graphs") {
 	    $("#graph_builder_phase_name").css("background", "#C43815");
		// $("#graph_builder_progress").progressbar("option", "disabled", false);
	} else if (data.phase_name == "Distributing vertex records") {
	 	$("#graph_builder_phase_name").css("background", "#4FA30C");
		// $("#graph_builder_progress").progressbar("option", "disabled", false);
	} else if (data.phase_name == "Running GraphLab") {
		$("#graph_builder_phase_name").css("background", "#6E00AC");
		if (($(".graph_builder").is(":visible"))) {
			$(".graph_builder").hide(500);
		}
		if (($("#graph_builder_progress").is(":visible"))) {
			$("#graph_builder_progress").hide(500);
		}
		$("#title").text("GraphLab Visualization Dashboard");
		return;
	} else {
		$("#graph_builder_phase_name").css("background", "#A353ED");
		// $("#graph_builder_progress").progressbar("option", "disabled", true);
    }

	if (!(data.phase_name == "Hadoop" || data.phase_name == "Finalizing graphs")) {
		if (!($("#graph_builder_progress").is(":visible"))) {
			$("#graph_builder_progress").show(500);
		}
	    $("#graph_builder_progress").progressbar({value: data.progress * 100});
	} else {
		if (($("#graph_builder_progress").is(":visible"))) {
			$("#graph_builder_progress").hide(500);
		}
